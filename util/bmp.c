...
ClImage* clLoadImage(char* path)
{
	...
	if (fileType == 0x4D42)
	{
		...
		if (bmpInfoHeader.biBitCount == 8)
		{
			...
			bmpImg->channels = 1;
			bmpImg->imageData = (signed char*)malloc(sizeof(unsigned char)*width*height);
			step = channels*width;
		...
		else if (bmpInfoHeader.biBitCount == 24)
		{
			...
			bmpImg->height = height;
			bmpImg->channels = 3;
			bmpImg->imageData = (signed char*)malloc(sizeof(unsigned char)*width*3*height);
			step = channels*width;
...
bool clSaveImage(char* path, ClImage* bmpImg)
{
	...
	else if (bmpImg->channels == 1)//8位，单通道，灰度图
	{
		...
		for(i=0;i<bmpImg->height;i++){
			fwrite(bmpImg->imageData+i*bmpImg->width, bmpImg->width, 1, pFile);
			if(offset)
				fwrite(&zero,offset,1,pFile);
		}
